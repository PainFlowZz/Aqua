...
module.exports.run = async (msg, message, args) => {
    const scembed = new Discord.RichEmbed()
    .setColor(rcolour)
    .setDescription("❯ Please choose a gender. !male or !female `❌`")

    if(!message.member.roles.find(r => r.name === "Female") && !message.member.roles.find(r => r.name === "Male")) return message.channel.send(scembed)
    ...
    .setDescription("Gay Lord")
    .setColor(colour)
    .setImage("https://media.tenor.com/images/524f1d581f51a30feb0a4df0bec761ef/tenor.gif")
    const lesembed = new Discord.RichEmbed()
    .setDescription("Lesbian")
    .setColor(colour)
    .setImage("https://media1.tenor.com/images/8d22b056b14dd9d4f6d91d4e81fb19e0/tenor.gif?itemid=10701769")       
    
    if (message.mentions.members.first().roles.find(r => r.name === "Male") && message.member.roles.find(r => r.name === "Male")) return message.channel.send(gayembed);
    if (message.mentions.members.first().roles.find(r => r.name === "Female") && message.member.roles.find(r => r.name === "Female")) return message.channel.send(lesembed);
